...
class MultiHeadedAttention_part(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        "Take in model size and number of heads."
        super(MultiHeadedAttention_part, self).__init__()
        assert d_model % h == 0
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, x, wordemb, imgsfeats):
         ...
         #print (y.size()) # 40 16 2048 8
         y = y.transpose(2,1).view(y.size(0), 512, 128) #40 2048 16 8
         y = y.unsqueeze(1).expand(y.size(0),20,512, 128) # 10, 30, 512, 41
         att = self.tanh(x + y + wordemb)
         att = att.permute(0, 1, 3, 2)
         #print (att.size())
         e = self.v(att).squeeze(3)
         alpha = self.softmax(e)  # 40, 20, 128
...
class convcap(nn.Module):
  ...
  def forward(self, imgsfeats, imgsfc7, wordclass):
    ...
    return x, multimodal_feature, attn_buffer, reconstruct
...
class Baseline(nn.Module):
    ...
    def __init__(self, num_classes, last_stride, model_path, neck, neck_feat, model_name, pretrain_choice):
        ...
        self.fc_init = nn.Linear(2048, 512)
        # self.gap = nn.AdaptiveMaxPool2d(1)
        self.num_classes = num_classes
        self.neck = neck
        self.neck_feat = neck_feat
        self.att = MultiHeadedAttention(8, 512)
        self.att_reconstruct = MultiHeadedAttention(8, 512)
        #self.att_reconstruct_part = MultiHeadedAttention_part(8, 2048)
        self.capmodel = convcap(num_class, num_layers = 3)
        self.fc_reconstruct = nn.Linear(512, 2048)
        if self.neck == 'no':
    ...
    def forward(self, x, language):
        f = self.base(x)
        f = f.permute(0,2,3,1)
        f = f.view(f.size(0), 128 , -1)
        f1 = F.relu(self.fc_init(f))
        f1_global,_ = torch.max(f1, 1)
        global_feat = self.gap(self.base(x))  # (b, 2048, 1, 1)
        global_feat = global_feat.view(global_feat.shape[0], -1)  # flatten to (bs, 2048)
        wordact, multimodal_feature, attmap, reconstruct = self.capmodel(f1, f1_global, language)
        reconstruct_source = reconstruct
        reconstruct = self.att_reconstruct(reconstruct, reconstruct, reconstruct)
        reconstruct = F.relu(self.fc_reconstruct(reconstruct)) 
        #reconstruct_part = self.att_reconstruct_part(reconstruct_source, reconstruct_source, reconstruct_source)
        ...
        if self.training:
            ...
            return cls_score, global_feat, wordact, reconstruct, global_feat  # global feature for triplet loss
        else: 
            if self.neck_feat == 'after':
                # print("Test with feature after BN")
                return feat, wordact, reconstruct
            else:
