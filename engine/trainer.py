...
import logging
import sys
sys.path.insert(0, "/home/ECIT.QUB.AC.UK/3054256/apex")
...
def create_supervised_trainer_with_center(model, center_criterion, optimizer, optimizer_center, loss_fn, cetner_loss_weight,
    ...
    def _update(engine, batch):
        ...
        language = language.to(device) if torch.cuda.device_count() >= 1 else language
        target = target.to(device) if torch.cuda.device_count() >= 1 else target
        score, feat, wordact, reconstruct, feats_whole = model(img, language)
        wordact = wordact[:,:,:-1]
        wordclass_v = language[:,1:]
        wordact_t = wordact.permute(0, 2, 1).contiguous().view(\
           wordact.size(0)*(20-1), -1)
        ...
        rl_loss = F.mse_loss(reconstruct.squeeze(), feats_whole.squeeze())
        #rl_loss_part = F.mse_loss(reconstruct_part, f.view(f.size(0), f.size(1), -1).transpose(2,1))
        loss_captioning = F.cross_entropy(wordact_t, \
               wordclass_t.contiguous()) + rl_loss# + rl_loss_part
        ...
        grad_captioning = torch.autograd.grad(loss_captioning, [value for para, value in model.named_parameters() if 'bn' not in para and 'classifier' not in para and 'att' not in para and 'capmodel' not in para and 'bottleneck' not in para and 'fc_reconstruct' not in para], allow_unused=True, retain_graph = True)
...
def do_train(
    ...
    RunningAverage(output_transform=lambda x: x[0]).attach(trainer, 'avg_loss_reid')
    RunningAverage(output_transform=lambda x: x[1]).attach(trainer, 'avg_loss_captioning')
    RunningAverage(output_transform=lambda x: x[2]).attach(trainer, 'avg_acc')
    ...
    def log_training_loss(engine):
        ...
        if ITER % log_period == 0:
            logger.info("Epoch[{}] Iteration[{}/{}] Loss_reid: {:.3f}, loss_captioing: {:.3f}, Acc: {:.3f}, Base Lr: {:.2e}"
                        .format(engine.state.epoch, ITER, len(train_loader),
                                engine.state.metrics['avg_loss_reid'], engine.state.metrics['avg_loss_captioning'], engine.state.metrics['avg_acc'],
                                scheduler.get_lr()[0]))
...
def do_train_with_center(
    ...
    def log_training_loss(engine):
        ...
        if ITER % log_period == 0:
            logger.info("Epoch[{}] Iteration[{}/{}] Loss_reid: {:.3f}, loss_captioning:{:.3f}, Acc: {:.3f}, Base Lr: {:.2e}"
                        .format(engine.state.epoch, ITER, len(train_loader),
                                engine.state.metrics['avg_loss_reid'], engine.state.metrics['avg_loss_captioning'], engine.state.metrics['avg_acc'],
                                scheduler.get_lr()[0]))
        if len(train_loader) == ITER:
